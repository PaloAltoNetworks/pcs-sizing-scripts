...
count_account_resources() {
  ...
  LAMBDA_CREDIT_USAGE=$((LAMBDA_COUNT_GLOBAL/6))
  ...
  TOTAL_BILLABLE_WITH_IAM_MODULE=$((${WORKLOAD_COUNT_GLOBAL_WITH_IAM_MODULE} + ${LAMBDA_CREDIT_USAGE}))

  echo "###################################################################################"
  echo ""
  echo "Resource counts CSPM:"
  ...
  echo "  Total billable CSPM credits: ${WORKLOAD_COUNT_GLOBAL}"
  echo "  Total billable IAM credits: ${WORKLOAD_COUNT_IAM_MODULE}"
  echo "  Total billable Compute credits: ${LAMBDA_CREDIT_USAGE}"
  echo "    ^ This is assuming all Lambda Functions will be scanned by Compute"
  echo ""
  echo "###################################################################################"
  echo "Grand Total billable credits: ${TOTAL_BILLABLE_WITH_IAM_MODULE}"
  echo "###################################################################################"
  echo ""
}
